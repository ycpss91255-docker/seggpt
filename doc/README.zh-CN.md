...
| Layer | 路径 | 职责 |
|---|---|---|
| Layer 1 | `src/runtime/` | Kernel — SegGPT model + service code（stateful `target` / `prompt` / `reset`）。内部使用，不对外暴露。 |
| Layer 2 | `src/api/` | 稳定 Python API：`SegGPTBackend.infer(target, refs, masks)` -> raw `{mask, class_id, inference_latency_ms, gpu_mem_mb}`。Stateless one-shot。下游 consumer 应该 import 的入口。 |
| Layer 3 | `src/server/` | FastAPI HTTP wrapper，包 Layer 2 给外部 client / 交互式测试使用。 |
