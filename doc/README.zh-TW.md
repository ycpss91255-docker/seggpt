...
| Layer | 路徑 | 職責 |
|---|---|---|
| Layer 1 | `src/runtime/` | Kernel — SegGPT model + service code（stateful `target` / `prompt` / `reset`）。內部用，不對外暴露。 |
| Layer 2 | `src/api/` | 穩定 Python API：`SegGPTBackend.infer(target, refs, masks)` -> raw `{mask, class_id, inference_latency_ms, gpu_mem_mb}`。Stateless one-shot。下游 consumer 應該 import 的入口。 |
| Layer 3 | `src/server/` | FastAPI HTTP wrapper，包 Layer 2 給外部 client / 互動式測試用。 |
