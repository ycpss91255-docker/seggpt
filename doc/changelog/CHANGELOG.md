...
- `scripts/phase0_driver.py` — multi-target / multi-N sweep driver. Loads the model once, iterates over `<targets-dir>/*.png × N ∈ {1,4,8}`, computes mIoU vs `<gt-dir>/<same-name>.png`, copies mIoU < 0.5 cases to `failures/`, and emits the main test-flow §6 layout (`meta.json` + `per_image.csv` + `stats.json` + `n_sweep.csv` + `pred_masks/` + `SUMMARY.md`). N subset selection is diversity-ordered per `iron_beam_prompt/README.md`.
