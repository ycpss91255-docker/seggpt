...
| Layer | パス | 役割 |
|---|---|---|
| Layer 1 | `src/seggpt/runtime/` | カーネル — SegGPT model + service code（stateful `target` / `prompt` / `reset`）。内部利用、外部に露出しない。 |
| Layer 2 | `src/seggpt/api/` | 安定した Python API: `SegGPTBackend.infer(target, refs, masks)` -> raw `{mask, class_id, inference_latency_ms, gpu_mem_mb}`。Stateless one-shot。下流 consumer が import すべきエントリポイント。 |
| Layer 3 | `src/seggpt/server/` | FastAPI HTTP wrapper、Layer 2 を包む。外部 client や対話的テスト用。 |
